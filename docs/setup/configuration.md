...
3.  [Combined assets](../cms/Markup/filter-theme.md) are not minified by default.
4.  [Safe mode](#safe-mode) is disabled by default.
...
The October platform and some marketplace plugins will implement changes in two stages to ensure overall stability and integrity of the platform. This means they have a _test build_ in addition to the default _stable build_.
...
> **Note**: The above command may need to be performed with System Administrator or _sudo_ privileges. It should also be performed after each system update or when a new plugin is installed.
