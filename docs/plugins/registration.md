...
2.  Define [user permissions](../backend/users.md).
3.  Add [settings pages](settings.md#backend-pages), [menu items](#navigation-menus), [lists](../backend/lists.md) and [forms](../backend/forms.md).
4.  Create [database table structures and seed data](updates.md).
5.  Alter [functionality of the core or other plugins](events.md).
6.  Provide classes, [back-end controllers](../backend/controllers-views-ajax.md), views, assets, and other files.
...
> **Note:** if you are developing a plugin for the [Marketplace](http://octobercms.com/help/site/marketplace), the [updates/version.yaml](updates.md) file is required.
...
The **Plugin.php** file, called the _Plugin registration file_, is an initialization script that declares a plugin's core functions and information. Registration files can provide the following:
...
registerMarkupTags()  | registers [additional markup tags](registration.md#extending-twig) that can be used in the CMS.
registerComponents()  | registers any [front-end components](components.md#component-registration) used by this plugin.
registerNavigation()  | registers [back-end navigation menu items](registration.md#navigation-menus) for this plugin.
registerPermissions()  | registers any [back-end permissions](../backend/users.md#permission-registration) used by this plugin.
registerSettings()  | registers any [back-end configuration links](settings.md#link-registration) used by this plugin.
registerFormWidgets()  | registers any [back-end form widgets](../backend/widgets.md#form-widget-registration) supplied by this plugin.
...
registerMailLayouts()  | registers any [mail view layouts](mail.md#mail-template-registration) supplied by this plugin.
registerMailTemplates()  | registers any [mail view templates](mail.md#mail-template-registration) supplied by this plugin.
registerMailPartials()  | registers any [mail view partials](mail.md#mail-template-registration) supplied by this plugin.
registerSchedule()  | registers [scheduled tasks](scheduling.md#defining-schedules) that are executed on a regular basis.
...
icon  | a name of the plugin icon. The full list of available icons can be found in the [UI documentation](https://octobercms.com/docs/ui/icon). Any icon names provided by this font are valid, for example **icon-glass, icon-music**.
...
Dependency definitions will affect how the plugin operates and [how the update process applies updates](updates.md#update-process). The installation process will attempt to install any dependencies automatically, however if a plugin is detected in the system without any of its dependencies it will be disabled to prevent system errors.
...
To make the sub-menu items visible, you may [set the navigation context](../backend/controllers-ajax.md#navigation-context) in the back-end controller using the `BackendMenu::setContext` method. This will make the parent menu item active and display the children in the side menu.
...
Next: [Version history](updates.md)

Previous: [Developing themes](../themes/development.md)
