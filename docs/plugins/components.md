...
depends  | an array of property names a dropdown property depends on. See the [dropdown properties](components.md#dropdown-properties) below.
...
Sometimes components need to create links to the website pages. For example, the blog post list contains links to the blog post details page. In this case the component should know the post details page file name (then it can use the [page Twig filter](../CMS/markup/README.md)). October includes a helper for creating dynamic dropdown page lists. The next example defines the postPage property which displays a list of pages:
...
Alternatively the value can be referenced dynamically from the page URL using an [external property value](../CMS/Components.md#external-property-values):
...
Like all methods in the [page execution life cycle](../CMS/Layouts.md#layout-life-cycle), if the `onRun` method in a component returns a value, this will stop the cycle at this point and return the response to the browser. Here we return an access denied message using the `Response` facade:
...
If the alias for this component was _demoTodo_ this handler can be accessed by `demoTodo::onAddItem`. Please see the [Calling AJAX handlers defined in components](../ajax/handlers.md#calling-handlers) article for details about using AJAX with components.
...
Components can reference themselves inside their partials by using the `__SELF__` variable. By default it will return the component's short name or [alias](../CMS/components.md#aliases).
...
Next: [Settings & Config](settings.md)

Previous: [Using Composer](composer.md)
