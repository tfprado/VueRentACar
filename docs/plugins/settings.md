...
Just like [setting navigation context in the controller](../backend/controllers-views-ajax.md#navigation-context), Back-end settings pages should set the settings navigation context. It's required in order to mark the current settings link in the System page sidebar as active. Use the `System\Classes\SettingsManager` class to set the settings context. Usually it could be done in the controller constructor:
...
Previous: [Building components](components.md)
