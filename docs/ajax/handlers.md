...
Every AJAX request should specify a handler name, either using the [data attributes API](attributes-api.md) or the [JavaScript API](javascript-api.md). When the request is made, the server will search all the registered handlers and locate the first one it finds.
...
You may want to use the [`__SELF__`](../plugin/components.md#referencing-self) reference variable instead of the hard coded alias in case the user changes the component alias used on the page.
...
> **Note**: When throwing this exception type [partials will be updated](update-partials.md) as normal.
...
Next: [Updating partials](update-partials.md)
