...
Like [raw queries](basics.md#running-queries), the `get` method returns an `array` of results where each result is an instance of the PHP `stdClass` object. You may access each column's value by accessing the column as a property of the object:
...
Duplicate queries across the same request can be prevented by using in-memory caching. This feature is enabled by default for [queries prepared by a model](model.md#retrieving-models) but not those generated directly using the `Db` facade.
...
Next: [Models](model.md)

Previous: [Structure](structure.md)