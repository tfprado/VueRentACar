...
> **Note:** Using the `Db` facade's transaction methods also controls transactions for the [query builder](query.md) and [model queries](model.md).
...
Just like [event registration](../services/events.md#event-registration), you may register your query listener in the `boot` method of a [Plugin registration file](../plugins/registration.md#registration-methods). Alternatively, plugins can supply a file named **init.php** in the plugin directory that you can use to place this logic.
...
> **Note**: For quicker debugging it may be more useful to call the `trace_sql` [helper function](../services/error-log.md#helpers) instead.

Next: [Structure](structure.md)

Previous: [Users & Permissions](../backend/users.md)
